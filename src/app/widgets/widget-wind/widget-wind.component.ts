...
import { pathRegistrationValue } from 'src/app/signalk.service';
...
export class WidgetWindComponent extends BaseWidgetComponent implements OnInit, OnDestroy  {
  ...
  subscribeHeading() {
    ...
    // let dataObservable: Observable<pathRegistrationValue> = this.signalKService.subscribePath(this.widgetProperties.uuid, this.widgetProperties.config.paths['headingPath'].path, this.widgetProperties.config.paths['headingPath'].source);

    // let sub: Subscription = null;
    sub = dataObservable.subscribe(
      newValue => {
