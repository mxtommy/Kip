...
describe('PluginConfigClientService', () => {
    ...
    it('should list plugins and normalize state', async () => {
        ...
        expect(result.data[0].state.enabled).toBe(true);
    ...
    it('should fallback to /plugins list when /plugins/{id} returns 404', async () => {
        ...
        const result = await promise;
        expect(result.ok).toBe(true);
        expect(result.capabilities.detailSupported).toBe(false);
        expect(result.capabilities.detailFallbackToList).toBe(true);
    });
    ...
    it('should normalize schema and flag unsupported keywords', () => {
        ...
        expect(normalized.hasSchema).toBe(true);
