...
describe('PathDiscoveryService', () => {
    ...
    it('should include already-cached paths immediately on register', () => {
        ...
        expect(service.activePaths(token).has('self.electrical.batteries.10.capacity.stateOfCharge')).toBe(true);
    });
    ...
    it('should add live paths from update stream', () => {
        ...
        expect(service.activePaths(token).has('self.electrical.batteries.11.capacity.stateOfCharge')).toBe(true);
    });
    ...
    it('should add path on state-only event', () => {
        ...
        expect(service.activePaths(token).has('self.electrical.batteries.12.capacity.stateOfCharge')).toBe(true);
    });
});
