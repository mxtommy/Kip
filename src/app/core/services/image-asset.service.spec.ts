  ...
  it('discovers the width allow-list from GET config and snaps against it', () => {
    const http = {
      ...
      get: vi.fn((url: string) =>
        url.endsWith('/config') ? of({ widthAllowlist: [100, 200, 400] }) : of([])
      ),
      delete: vi.fn(() => of({ ok: true }))
    };
    const connection = {
      serverServiceEndpoint$: new BehaviorSubject<{ httpServiceUrl: string | null }>({
        httpServiceUrl: 'http://host:3000/signalk/v1/api/'
      }),
      signalKURL: { url: '' }
    };
    TestBed.configureTestingModule({
