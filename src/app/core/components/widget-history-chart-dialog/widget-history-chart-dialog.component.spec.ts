...
describe('WidgetHistoryChartDialogComponent', () => {
  ...
  let historyApiClientMock: { getValues: jasmine.Spy };
  let historyMapperMock: { mapValuesToChartDatapoints: jasmine.Spy };
  ...
  beforeEach(async () => {
    historyApiClientMock = {
      getValues: jasmine.createSpy('getValues').and.callFake(({ paths }: { paths: string }) => Promise.resolve({
        data: [{}],
        requestedPath: paths
      }))
    };
    ...
    historyMapperMock = {
      mapValuesToChartDatapoints: jasmine.createSpy('mapValuesToChartDatapoints').and.callFake((response: { requestedPath?: string }) => {
