// Utility helpers for widget.service.ts mutation formatting & insertion logic
// Extracted to keep schematic index.ts lean and testable.

export interface WidgetDefBuildOptions {
...
export function buildWidgetDefinitionObject(selector: string, className: string, opts: WidgetDefBuildOptions): string {
  return `{
    name: '${opts.title}',
...
export function appendImport(src: string, importStatement: string): string {
...
export function updateComponentMap(src: string, className: string): string {
...
export interface DefinitionArrayInsertOptions extends WidgetDefBuildOptions { category: string; }

export function insertDefinitionObject(src: string, selector: string, className: string, opts: DefinitionArrayInsertOptions): string {
  ...
  return src.replace(defArrayPattern, (m, inner) => {
    ...
    metas.forEach((meta, idx) => {
      rebuilt += meta.text.endsWith('\n') ? meta.text : meta.text + '\n';
      if (idx === insertAfter) rebuilt += formattedNew;
    });
    if (insertAfter === -1) rebuilt += formattedNew;
    rebuilt = normalizeDefinitionFormatting(rebuilt, baseIndent);
    return m.replace(inner, rebuilt);
  });
}

export function normalizeDefinitionFormatting(content: string, baseIndent: string): string {
  let rebuilt = content;
...
export function escapeSingle(str: string): string {
